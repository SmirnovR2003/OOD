        ...
        :M_CREATE_SHAPES_FUNCTIONS(
            {
                ...
                {"regularpolygon", std::bind_front(&CShapeFactory::CreateRegularPolygon, this) },
            })
    {
    }
...
private:
    std::shared_ptr<CShape> CreateShape(const std::string& descr)
    {
        ...
        return M_CREATE_SHAPES_FUNCTIONS.find(splitedDescr[0])->second(splitedDescr);
    }
    std::shared_ptr<CShape> CreateEllipse(const std::vector<std::string>& descr)
    {
        ...
        if (M_COLORS.count(descr[1]) == 0)
        {
            throw std::invalid_argument("invalid ellipse color");
        }
        auto color = M_COLORS.find(descr[1])->second;
    ...
    const std::map<std::string, sf::Color> M_COLORS
    {
    ...
    const std::map<std::string, std::function<std::shared_ptr<CShape>(const std::vector<std::string>& descr)>> M_CREATE_SHAPES_FUNCTIONS;
